...
function renderQuestion() {
  ...
  // TEST 3: problem-solving answer entry.
  // correct_option may contain multiple answers separated by | (e.g. 12|24).
  // This keeps the existing database/RPC connection intact.
  if (currentTestMeta && currentTestMeta.testType === 'daily' && currentTestMeta.answerMode === 'problem') {
    const existing = answers[currentIndex];
    const expectedParts = String(q.correct_option || '').split('|');
    const count = Math.max(1, expectedParts.length);
    let values = [];
    if (Array.isArray(existing)) values = existing;
    else if (existing !== null && existing !== undefined && existing !== '') values = String(existing).split('|');

    $('options').innerHTML = Array.from({length: count}, (_, i) => `
      <div class="answer-entry">
        <label class="answer-label">${count === 1 ? 'अपना अंतिम उत्तर लिखें' : `उत्तर ${i + 1}`}</label>
        <input type="text" class="problem-answer" data-answer-index="${i}" inputmode="decimal"
          placeholder="यहाँ अपना उत्तर लिखें" value="${esc(values[i] || '')}">
      </div>
    `).join('');

    $('options').querySelectorAll('.problem-answer').forEach(input => {
  ...
  // TEST 1 / TEST 2: existing MCQ interface remains unchanged.
...
function getAttemptLockKey() {
  ...
  // Daily: one attempt per day for the student's class, regardless of
  // which studied chapter range was selected before starting.
  if (m.testType === 'daily') {
    return `ganit_setu_lock_${student}_daily_${m.classLevel}_${getLocalDateKey()}`;
  }
...
async function ensureTestIsUnlocked(testId) {
  ...
  const alreadyAttempted = (data || []).some(a => {
    ...
    // THIRD / DAILY TEST:
    // One attempt per day for this class, regardless of selected chapter range.
    if (m.testType === 'daily') {
      return String(t.test_type) === 'daily' &&
             Number(t.class_level) === Number(m.classLevel) &&
             String(t.title).includes(getLocalDateKey());
    }
...
async function saveAttempt() {
  ...
  const payload = questions.map((q,i) => ({
    question_id: q.id,
    selected_option: Array.isArray(answers[i]) ? answers[i].map(v => String(v).trim()).join('|') : (answers[i] || null)
  }));
...
function beginTest(testBoxId, messageBoxId) {
  ...
  $('nextQuestion').onclick = () => {
    ...
    const isProblemAnswer = currentTestMeta && currentTestMeta.testType === 'daily' && currentTestMeta.answerMode === 'problem';
    if (isProblemAnswer) {
      const values = Array.isArray(currentAnswer) ? currentAnswer : [currentAnswer];
...
// TEST 3: 5 problem-solving questions from chapters 1..selected chapter.
function setupDailyTest() {
  if (!$('dailyStartBtn')) return;

  const classLevel = getClassLevel();
  const select = $('dailyChapterTo');
  if (!select) return;

  select.innerHTML = '<option value="">अध्याय चुनें</option>';
  for (let n=1; n<=MAX_CHAPTERS[classLevel]; n++) {
    select.innerHTML += `<option value="${n}">अध्याय 1 से ${n} तक</option>`;
  }

  if ($('dailyClassInfo')) {
    $('dailyClassInfo').textContent = `कक्षा ${classLevel} • अधिकतम ${MAX_CHAPTERS[classLevel]} अध्याय`;
  }
  ...
  $('dailyStartBtn').onclick = async () => {
    ...
    if (!chapterTo) {
      message.innerHTML = '<div class="error-box">कृपया पहले बताएं कि आपने कितने अध्याय तक पढ़ाई की है।</div>';
      return;
    }

    try {
      const all = await loadQuestions(classLevel);
      const studiedQuestions = all.filter(q => Number(q.chapter_number) >= 1 && Number(q.chapter_number) <= chapterTo);

      if (studiedQuestions.length < DAILY_QUESTION_COUNT) {
        throw new Error(`चयनित अध्यायों में अभी ${studiedQuestions.length} Questions हैं। Test 3 के लिए कम से कम ${DAILY_QUESTION_COUNT} Questions उपलब्ध होने चाहिए।`);
      }

      const shuffled = [...studiedQuestions];
      ...
      if ($('dailyTitle')) {
        $('dailyTitle').textContent =
          `कक्षा ${classLevel} • अध्याय 1 से ${chapterTo} • 5 प्रश्न`;
      }
